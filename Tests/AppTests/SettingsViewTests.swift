...
struct SettingsViewTests {
    ...
    func fullSettingsWindowKeepsListAndApplyInsideTheSelectedItemsTab(style: FloatingBarStyle, scheme: ColorScheme) async throws {
        ...
        func assertFit(withReadError: Bool = false) throws {
            #expect(hosting.view.bounds.size == CGSize(width: 640, height: 720))
            #expect(window.contentLayoutRect.size == CGSize(width: 640, height: 720))
            ...
            #expect(itemsFrame.maxY <= headerFrame.minY)
            ...
            #expect(listFrame.height >= 120)
            #expect(itemsFrame.contains(listFrame))
